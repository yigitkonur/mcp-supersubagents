...
## Current State
- Timeout defaults are configured in `src/config/timeouts.ts` and used by validation in `src/utils/sanitize.ts` (default 10 minutes).
...
2. **Default timeout is short**: 10 minutes can be too aggressive for large tasks, increasing timeouts and user frustration.
