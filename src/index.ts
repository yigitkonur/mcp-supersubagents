...
server.setRequestHandler(ReadResourceRequestSchema, async (request) => {
   ...
   */
  function renderTaskMarkdown(task: TaskState, msgStats: { round: number; totalMessages: number }, filtered: string[]): string {
  ...
  return {
    contents: [{
      uri,
      mimeType: 'text/markdown',
      text: renderTaskMarkdown(task, msgStats, filtered),
    }],
  };
  } catch (err) {
