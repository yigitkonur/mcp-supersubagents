...
- **Recommended Fix:** Before spawning, check if the session is still alive. If the session was destroyed or the original task used Claude fallback, spawn a fresh task with a handoff prompt instead of attempting resume.
...
- **Recommended Fix:** Add a `retryInProgress` flag per-task. Set before triggering retry, clear on completion/failure.
...
- **Recommended Fix:** Preserve existing retryCount: `retryCount: (task.retryInfo?.retryCount ?? 0)`.
...
- **Recommended Fix:** Build the snapshot BEFORE unbinding/destroying the session. Add minimum-content check.
...
- **Recommended Fix:** Add `return;` after the `shouldFallbackToClaudeCode` block.
...
- **Recommended Fix:** Increment `rotationCount` and set `lastRotationTime` when `getCurrentToken()` switches tokens.
...
- **Recommended Fix:** Don't increment `rotationAttempts` for proactive rotations, or use a separate counter.
...
- **Recommended Fix:** In `abortAllFallbackSessions`, drain `fallbackQueue` by resolving all pending resolvers.
...
- **Recommended Fix:** Add pre-flight check at startup or in `triggerClaudeFallback`.
...
- **Recommended Fix:** Key the guard on `sessionId` in addition to `taskId`.
...
- **Severity:** Low
- **Status:** ⚠️ Open (partially mitigated by FB-023 fix)
...
- **Recommended Fix:** Use SDK's `quotaInfo.resetDate` when available (now partially addressed by FB-023 fix). Fallback to a longer default (5 minutes).
...
- **Recommended Fix:** Use structured SDK `errorType` field instead of string matching.
...
- **Recommended Fix:** Store rate limit reset timestamp from `quotaInfo.resetDate` in `TokenState.resetAt`.
...
- **Recommended Fix:** Consider returning `retryAfterMs` hint instead of `allExhausted`.
...
- **Recommended Fix:** Only skip lines matching known system prefixes.
...
- **Recommended Fix:** Add `task.fallbackPending: boolean` flag for monitoring.
...
- **Recommended Fix:** Document this behavior in the `spawn_agent` tool description.
