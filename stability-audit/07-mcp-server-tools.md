...
- **Fix Applied:** Add matching cleanup for `statusUpdateTimers`.
...
- **Fix Applied:** Check `isShuttingDown` in `createTask()` or at handler top.
...
- **Fix Applied:** Wrap each resource handler branch in `try/catch`.
...
- **Fix Applied:** Wrap `console.error` in shutdown with `try/catch`.
...
- **Fix Applied:** Re-read and re-validate the task after acquiring the `resumeInProgress` lock.
...
- **Fix Applied:** Wrap the entire handler in try/catch.
...
- **Fix Applied:** Queue status change notifications and flush after `updateTask()` completes.
...
- **Fix Applied:** Guard the return with a null check.
...
- **Fix Applied:** Validate parsed cursor and throw on NaN.
...
- **Fix Applied:** Add null guard inside `.map()`.
...
- **Fix Applied:** Add `progressRegistry.clear()` to shutdown path.
...
- **Fix Applied:** Call `subscriptionRegistry.clear()` in shutdown and on stdin disconnect.
...
- **Fix Applied:** On execute failure, transition the task to FAILED.
...
- **Fix Applied:** Register stdin handlers before `server.connect()`.
...
- **Fix Applied:** Add a rate limiter (e.g., exit after 10 uncaught exceptions in quick succession).
...
- **Fix Applied:** Pass `model`, `autonomous`, and `taskType` from original task.
...
- **Fix Applied:** Use `new URL(r.uri).pathname` for cross-platform support.
...
- **Fix Applied:** Track retry count per task and decrement.
