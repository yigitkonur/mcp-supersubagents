...
export const SpawnTaskSchema = z.object({
    ...
    model: z.string().refine((val) => !val || MODEL_IDS.includes(val), { message: `Invalid model. Valid models: ${MODEL_IDS.join(', ')}` }).optional(),
    task_type: z.string().refine((val) => !val || TASK_TYPE_IDS.includes(val), { message: `Invalid task_type. Valid types: ${TASK_TYPE_IDS.join(', ')}` }).optional(),
